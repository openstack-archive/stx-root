...
Options:
    ...
    --base:       Specify base docker image
    --version:    Specify version for output image
    --release:    Openstack release (default: pike)
    --repo:       Software repository (Format: name,baseurl)
    --local:      Use local build for software repository
...
OPTS=$(getopt -o h -l help,os:,version:,release:,repo:,push,user:,registry:,release:,base:,wheels:,only:,skip:,latest,clean -- "$@")
if [ $? -ne 0 ]; then
    usage
    exit 1
fi
...
if [ -z "${IMAGE_VERSION}" ]; then
    # Auto-generate a version using timestamp
    IMAGE_VERSION=$(date --utc '+%Y.%m.%d.%H.%M')
fi
...
if [ "${CLEAN}" = "yes" ]; then
    ...
    docker image rm ${BASE} ${RESULTS_BUILT[@]} ${RESULTS_PUSHED[@]}
    if [ $? -ne 0 ]; then
