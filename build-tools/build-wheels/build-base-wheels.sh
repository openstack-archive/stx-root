...
function usage {
    cat >&2 <<EOF
Usage:
$(basename $0) [ --os <os> ] [ --keep-image ] [ --keep-container ]
...
OPTS=$(getopt -o h -l help,os:,keep-image,keep-container -- "$@")
if [ $? -ne 0 ]; then
    usage
    exit 1
fi
...
BUILD_OUTPUT_PATH=${MY_WORKSPACE}/std/build-wheels-${OS}/base
BUILD_IMAGE_NAME="${USER}-$(basename ${MY_WORKSPACE})-wheelbuilder:${OS}"
...
docker build -t ${BUILD_IMAGE_NAME} -f ${DOCKER_PATH}/${OS}-dockerfile ${DOCKER_PATH}
if [ $? -ne 0 ]; then
    echo "Failed to create build image in docker" >&2
    exit 1
fi

# Run the image, excuting the build-wheel.sh script
RM_OPT=
