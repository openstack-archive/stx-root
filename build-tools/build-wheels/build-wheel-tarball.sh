...
function get_current_version {
    ...
    curl -H "Authorization: Bearer ${token}" -k -sSL -X GET https://${registry_server}/v2/${REPO_USER}/${LABEL}/tags/list \
        | python -c '
...
OPTS=$(getopt -o h -l help,os:,push,user: -- "$@")
if [ $? -ne 0 ]; then
    usage
    exit 1
fi
...
${MY_SCRIPT_DIR}/build-base-wheels.sh --os ${OS}
if [ $? -ne 0 ]; then
    echo "Failure running build-base-wheels.sh" >&2
    exit 1
fi

${MY_SCRIPT_DIR}/get-stx-wheels.sh --os ${OS}
if [ $? -ne 0 ]; then
    echo "Failure running get-stx-wheels.sh" >&2
    exit 1
fi

BUILD_OUTPUT_PATH=${MY_WORKSPACE}/std/build-wheels-${OS}/tarball
...
IMAGE_NAME=stx-${OS}-wheels

TARBALL_FNAME=${MY_WORKSPACE}/std/build-wheels-${OS}/${IMAGE_NAME}.tar
if [ -f ${TARBALL_FNAME} ]; then
    rm -f ${TARBALL_FNAME}
fi

# Download the global-requirements.txt and upper-constraints.txt files
wget https://raw.githubusercontent.com/openstack/requirements/stable/pike/global-requirements.txt
if [ $? -ne 0 ]; then
    echo "Failed to download global-requirements.txt" >&2
    exit 1
fi

wget https://raw.githubusercontent.com/openstack/requirements/stable/pike/upper-constraints.txt
if [ $? -ne 0 ]; then
