...
for helm_rpm in "${HELM_RPMS[@]}"; do
    # Bash globbing does not handle [^-] like regex, so grep needed to be used
...
# The new tarball contains armada files at the top level
# and the helm charts in a subdir called charts
#tar --transform 's/.*\///g' ${TAR_FLAGS} helm-charts.tgz usr/lib/helm/*.tgz
mkdir staging
cp usr/lib/armada/* staging/.
cp -R usr/lib/helm staging/charts
tar --transform 's/staging\///g' ${TAR_FLAGS} helm-charts.tgz staging
