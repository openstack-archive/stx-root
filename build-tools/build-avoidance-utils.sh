...
date_to_iso_8601 () {
    ...
    for x in $(echo "${BUILD_AVOIDANCE_DATE_FORMAT}" | tr ' ' '#' | sed 's/%%/#/g' | tr '%' ' ' ); do
        ...
        case $x in
            ...
            *) >&2 echo "$FUNCNAME (${LINENO}): Unsuported date format: ${BUILD_AVOIDANCE_DATE_FORMAT}"; return 1;;
        esac
...
time_to_iso_8601 () {
    ...
    for x in $(echo "${BUILD_AVOIDANCE_TIME_FORMAT}" | tr ' ' '#' | sed 's/%%/#/g' | tr '%' ' ' ); do
        ...
        case $x in
            ...
            *) >&2 echo "$FUNCNAME (${LINENO}): Unsuported time format: ${BUILD_AVOIDANCE_TIME_FORMAT}"; return 1;;
        esac
    ...
    if [ "$H12" != "" ] && [ "$AMPM" != "" ]; then
        ...
        if [ "$H12" != "" ] && [ "$AMPM" != "" ]; then
            >&2 echo "$FUNCNAME (${LINENO}): Unsuported time format: ${BUILD_AVOIDANCE_TIME_FORMAT}"
            return 1
        fi
    fi
